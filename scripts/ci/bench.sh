...
if [ "$CI" == "true" ] ; then
	...
	go run -modfile=scripts/ci/ci.go.mod github.com/elastic/gobench -index "benchmark-go" -es "${APM_AGENT_GO_CLOUD_SECRET}" < ${OUT_FILE} || echo "Warning: Failed to send benchmark data to Elasticsearch. Benchmarks completed successfully."
fi
