...
If you are using Echo 4.x (`github.com/labstack/echo/v4`), then use `module/apmechov4`.
...
https://golang.org/pkg/net/http/#Request.WithContext[http.Request.WithContext], or a helper provided by https://golang.org/x/net/context/ctxhttp.

Client spans are not ended until the response body is fully consumed or closed. The span will not be sent if these are not done. Close the response body to ensure HTTP connections can be reused; see https://golang.org/pkg/net/http/#Client.Do.
