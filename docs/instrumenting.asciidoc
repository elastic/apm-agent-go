...
To report `pgx` queries, create `pgx` config, and then assign `apmpgx.NewTracer()` to config logger. Also you can
pass logger you want to as argument for `apmpgx.NewTracer()` (it's safe to pass `nil`)
...
func main() {
    config, err := pgxpool.ParseConfig("dsn_string")
    ...
    config.ConnConfig.Logger = apmpgx.NewTracer(nil)
