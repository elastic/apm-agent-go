...
[[builtin-modules-apmatreugo]]
==== module/apmatreugo
Package apmatreugo provides the middleware and panic view for the https://github.com/savsgio/atreugo[savsgio/atreugo]
web middleware.

For each request, a transaction is stored in the request context, which can be obtained via
https://pkg.go.dev/github.com/savsgio/atreugo/v11#RequestCtx[atreugo.RequestCtx] in your handler using `apm.TransactionFromContext`.
...
import (
	...
	"go.elastic.co/apm/module/apmatreugo"
)

func main() {
	apmFactory := apmatreugo.New()

	s := atreugo.New(atreugo.Config{
		Addr:      ":8000",
		PanicView: apmFactory.PanicView(),
	})

	s.UseBefore(apmFactory.Middleware())
}
----

The apmatreugo panic view will recover panics and send them to Elastic APM.
