...
ParentID returns the transaction's Parent ID generating a parent span ID if one has
not already been set and tx has not been ended.
...
[source,go]
----
// TraceContext returns a map[string]interface{} containing the trace
...
func TraceContext(ctx context.Context) map[string]interface{} {
	...
	fields := map[string]interface{}{
		"trace_id":              traceContext.Trace,
		"transaction_id":        traceContext.Span,
		"transaction_parent_id": tx.ParentID(),
	}
	if span := apm.SpanFromContext(ctx); span != nil {
		fields["span_id"] = span.ParentID()
	}
	return fields
}
----
...
ParentID returns the span's Parent ID generating a parent span ID if one has
not already been set and tx has not been ended.
If tx is nil or has been ended, a zero (invalid) SpanID is returned.
