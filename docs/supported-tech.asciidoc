...
We support several third-party web frameworks, as well as Go's standard `net/http` package or the high-performance `valyala/fasthttp` implementation package. Regardless of the framework, we create a transaction for each incoming
request, and name the transaction after the registered route.

[float]
==== fasthttp

We support the https://github.com/valyala/fasthttp[valyala/fasthttp],
