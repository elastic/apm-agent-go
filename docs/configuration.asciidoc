...
Adapt the Elastic APM GO {agent} by one of the following methods, in descending order of precedence: 
...
that the server certificate can be verified. Disable certificate
...
The timeout for requests made to your Elastic APM server. Tiemouts will be disable when this is set to zero or a negative value. 
...
This base64-encoded string is used to ensure that only your {agent}s can send data to your APM server.
...
By default, the agent verifies the servers certificate if you use an
