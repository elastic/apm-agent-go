...
// WithRecovery.
func Wrap(h http.HandlerFunc, route string, o ...Option) buffalo.Handler {
	opts := gatherOptions(o...)
	return buffalo.WrapHandler(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		if !opts.tracer.Active() || opts.requestIgnorer(req) {
			h(w, req)
			return
		}
		tx, req := apmhttp.StartTransaction(opts.tracer, req.Method+" "+massageTemplate(route), req)
		defer tx.End()
